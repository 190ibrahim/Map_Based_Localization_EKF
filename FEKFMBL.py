...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        if hf.size != 0:
            h_mf = np.vstack([h_mf, hf])  # Only measurements
        if hm.size != 0:
            h_mf = np.vstack([h_mf, hm])   # Only features
        return h_mf
    ...
    def SquaredMahalanobisDistance(self, hfj, Pfj, zfi, Rfi):
        ...
        Sij = Rfi + Pfj  # Covariance matrix of the difference
    ...
    def IndividualCompatibility(self, D2_ij, dof, alpha):
        ...
        """
        # TODO: To be completed by the student
        threshold = scipy.stats.chi2.ppf(1 - alpha, dof)  # Chi-Square threshold
    ...
    def ICNN(self, hf, Phf, zf, Rf, dim):
        ...
        for j in range(self.zfi_dim):
            ...
            for i in range(self.nf):
                ...
                if self.IndividualCompatibility(D2_ij, self.zfi_dim, self.alpha):
                    if D2_ij < D2_min:
                        nearest = i
                        D2_min = D2_ij
    ...
    def DataAssociation(self, xk, Pk, zf, Rf):
        ...
        # Step 1: Compute expected observations and covariances
        for i in range(self.nf):  # Loop over map features
            h_fi = self.hfj(xk, i)  # Expected observation for feature i
            J_hfi_x = self.Jhfjx(xk, i)  # Jacobian of observation function
            P_fi = J_hfi_x @ Pk @ J_hfi_x.T  # Covariance for feature i
    ...
    def Localize(self, xk_1, Pk_1):
        ...
        self.xk_bar, self.Pk_bar = self.Prediction(uk, Qk, xk_1, Pk_1)
        ...
        if len(zf) == 0:
            ...
            self.Hp = self.DataAssociation(self.xk_bar, self.Pk_bar, zf, Rf)

        zk, Rk, Hk, Vk, znp, Rnp = self.StackMeasurementsAndFeatures(zm, Rm, Hm, Vm, zf, Rf, self.Hp)
        if len(zk)!= 0:
            self.xk, self.Pk = self.Update(zk, Rk, self.xk_bar, self.Pk_bar, Hk, Vk)
        else:
    ...
    def StackMeasurementsAndFeatures(self, zm, Rm, Hm, Vm, zf, Rf, H):
        ...
        Hk = np.array([]).reshape(0,self.xB_dim)
        Vk = np.array([]).reshape(0,0)

        if len(zm) == 0:
            # No measurements, only paired features
            zk = zp
            Rk = Rp
            Hk = Hp
            Vk = Vp
            ...
            # No paired features, only measurements
            zk = zm
            Rk = Rm
            Hk = Hm
            Vk = Vm
        else:
            # Combine measurements and paired features
            zk = np.vstack((zm, zp))
    ...
    def SplitFeatures(self, zf, Rf, H):
        ...
        for i, h in enumerate(H):
            if h is not None:
                ...
                zp.append(zf[i])
                Rp.append(Rf[i])
                Hp.append(self.Jhfjx(self.xk_bar, h))
        ...
        if len(zp) > 0:
            zp = np.vstack(zp)
            Rp = scipy.linalg.block_diag(*Rp)
            Hp = np.vstack(Hp)
            Vp = scipy.linalg.block_diag(*Vp)
        znp = np.array(znp)
        Rnp = scipy.linalg.block_diag(*Rnp) if Rnp else np.array([])
    ...
    def PlotRobotUncertainty(self):  # plots the robot trajectory and its uncertainty ellipse
        ...
        """
        # Plot Robot Ellipse
        robot_ellipse = GetEllipse(self.xk, self.GetRobotPoseCovariance(self.Pk))
